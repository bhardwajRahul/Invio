...
function findFiles(dir, extensions = ['.svelte', '.ts', '.js']) {
  ...
  for (const item of items) {
    ...
    if (item.isDirectory()) {
      ...
      if (!['node_modules', 'build', '.svelte-kit', 'dist'].includes(item.name)) {
...
function extractAndSync() {
  ...
  console.log(`🔑 Found ${sortedKeys.length} unique translation keys in ${fileCount} files\n`);
  ...
  // Find missing keys
  const missingKeys = sortedKeys.filter(key => !enTranslations.hasOwnProperty(key));
  const unusedKeys = Object.keys(enTranslations).filter(key => !allKeys.has(key));
  ...
  if (unusedKeys.length > 0) {
    console.log(`🗑️  Removing unused keys from en.json (${unusedKeys.length} keys):`);
  ...
  if (missingKeys.length > 0 || unusedKeys.length > 0) {
    ...
    const otherFiles = fs.readdirSync(LOCALES_DIR)
