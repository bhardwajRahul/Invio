...
type Data = {
  ...
  money?: { billed: number; paid: number; outstanding: number; currency: string };
...
export const handler: Handlers<Data> = {
  async GET(req, ctx) {
    ...
    try {
      ...
      return ctx.render({
        authed: true,
        counts: { invoices: invoices.length, customers: customers.length, templates: templates.length },
            ...
            <table class="table table-zebra w-full text-sm">
              ...
              <tbody>
                {props.data.recent.map((inv) => {
                  ...
                  const badge = inv.status === "paid" ? "badge-success" : inv.status === "overdue" ? "badge-error" : inv.status === "sent" ? "badge-info" : "";
                  return (
                    <tr class="hover" key={inv.id}>
                      ...
                      <td class="text-right"><a class="btn btn-ghost btn-sm" href={`/invoices/${inv.id}`}>Open</a></td>
                    </tr>
                  );
                })}
              </tbody>
