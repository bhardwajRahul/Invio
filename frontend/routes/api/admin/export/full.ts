...
export const handler: Handlers = {
  ...
  const authFromCookie = getAuthHeaderFromCookie(req.headers.get("cookie") || undefined) || undefined;
  const auth = authFromHeader || authFromCookie;
    if (!auth) {
      return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: { "content-type": "application/json" } });
    }
  const backendUrl = `${BACKEND_URL}/api/v1/export/full?includeDb=${includeDb}&includeJson=${includeJson}&includeAssets=${includeAssets}`;
  const resp = await fetch(backendUrl, { headers: { Authorization: auth } });
    if (!resp.ok) {
      const body = await resp.text().catch(() => "");
      return new Response(body || `Backend error ${resp.status}`, { status: resp.status, headers: { "content-type": resp.headers.get("content-type") || "text/plain" } });
    }
