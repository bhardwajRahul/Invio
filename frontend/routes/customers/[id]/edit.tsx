      ...
      {c && (
        <form method="post" class="space-y-3 max-w-xl bg-base-100 border p-4 rounded-box">
          <label class="form-control">
            ...
            <input name="name" value={c.name || ""} class="input input-bordered w-full" />
          </label>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <label class="form-control">
              <div class="label"><span class="label-text">Email</span></div>
              <input type="email" name="email" value={c.email || ""} class="input input-bordered w-full" />
            </label>
            <label class="form-control">
              <div class="label"><span class="label-text">Phone</span></div>
              <input name="phone" value={c.phone || ""} class="input input-bordered w-full" />
            </label>
          </div>
          <label class="form-control">
            <div class="label"><span class="label-text">Address</span></div>
            <textarea name="address" class="textarea textarea-bordered" rows={3}>{c.address || ""}</textarea>
          </label>
          <label class="form-control">
            <div class="label"><span class="label-text">Tax ID</span></div>
            <input name="taxId" value={c.taxId || ""} class="input input-bordered w-full" />
          </label>
          <div class="pt-2">
