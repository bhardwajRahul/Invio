<script lang="ts">
  ...
  // Categories
  function handleAddCategory() { editingCategoryId = null; categoryForm = { code: "", name: "" }; showCategoryForm = true; }
  function handleEditCategory(cat: any) { editingCategoryId = cat.id; categoryForm = { code: cat.code, name: cat.name }; showCategoryForm = true; }
  function handleCancelCategory() { showCategoryForm = false; editingCategoryId = null; }
  ...
  async function handleSubmitCategory(e: SubmitEvent) {
    ...
    try {
      ...
      const res = await fetch(url, {
        method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(categoryForm)
      });
      if (!res.ok) throw new Error("API error");
      handleCancelCategory();
      invalidateAll();
    } catch(err) { alert(err); } finally { isSubmitting = false; }
  }
  ...
  // Units
  function handleAddUnit() { editingUnitId = null; unitForm = { code: "", name: "" }; showUnitForm = true; }
  function handleEditUnit(unit: any) { editingUnitId = unit.id; unitForm = { code: unit.code, name: unit.name }; showUnitForm = true; }
  function handleCancelUnit() { showUnitForm = false; editingUnitId = null; }
  ...
  async function handleSubmitUnit(e: SubmitEvent) {
    ...
    try {
      ...
      const res = await fetch(url, {
        method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(unitForm)
      });
      if (!res.ok) throw new Error("API error");
      handleCancelUnit();
      invalidateAll();
...
<div class="space-y-8">
  <!-- Categories -->
  <div class="space-y-4">
    <div class="flex items-center justify-between border-b pb-2 border-base-300">
      ...
      <button type="button" onclick={handleAddCategory} class="btn btn-sm btn-primary" disabled={demoMode}>
        <Plus size={16} class="mr-1" /> {t("Add Category")}
      </button>
    </div>

    {#if showCategoryForm}
      <form onsubmit={handleSubmitCategory} class="card bg-base-200 p-4 space-y-3">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <label class="form-control">
            <div class="label"><span class="label-text">{t("Category Code")} *</span></div>
            <input type="text" class="input input-bordered w-full" bind:value={categoryForm.code} required disabled={demoMode} />
          </label>
          <label class="form-control">
            <div class="label"><span class="label-text">{t("Category Name")} *</span></div>
    ...
    <div class="space-y-2">
      {#each productCategories as cat}
        <div class="flex items-center justify-between p-3 border border-base-300 rounded-box bg-base-100">
          <div><span class="font-medium">{cat.code}</span> <span class="text-base-content/60">&mdash; {cat.name}</span></div>
          <div class="flex gap-2">
      ...
      {#if productCategories.length === 0}
        <div class="text-center py-4 text-base-content/60">{t("No categories defined")}</div>
      {/if}
    </div>
  </div>
  ...
  <div class="space-y-4">
    <div class="flex items-center justify-between border-b pb-2 border-base-300">
      ...
      <button type="button" onclick={handleAddUnit} class="btn btn-sm btn-primary" disabled={demoMode}>
        <Plus size={16} class="mr-1" /> {t("Add Unit")}
      </button>
    </div>

    {#if showUnitForm}
      <form onsubmit={handleSubmitUnit} class="card bg-base-200 p-4 space-y-3">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <label class="form-control">
            <div class="label"><span class="label-text">{t("Unit Code")} *</span></div>
            <input type="text" class="input input-bordered w-full" bind:value={unitForm.code} required disabled={demoMode} />
          </label>
          <label class="form-control">
            <div class="label"><span class="label-text">{t("Unit Name")} *</span></div>
    ...
    <div class="space-y-2">
      {#each productUnits as unit}
        <div class="flex items-center justify-between p-3 border border-base-300 rounded-box bg-base-100">
          <div><span class="font-medium">{unit.code}</span> <span class="text-base-content/60">&mdash; {unit.name}</span></div>
          <div class="flex gap-2">
      ...
      {#if productUnits.length === 0}
        <div class="text-center py-4 text-base-content/60">{t("No units defined")}</div>
      {/if}
    </div>
  </div>
</div>
